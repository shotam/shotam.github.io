...
# read data
Exp4 = read.csv(paste0(wd, exp, 'data.csv'), header=T)
Exp4 = recode.vars(Exp4, c("PrimeType", "Homogeneity"), contr.sum, scaleLevelDiffsTo1=T)
Exp4$PrimeType.lev = -1*Exp4$PrimeType.lev


### descriptive stats
table(Exp4$VerbClass)
Subj <- aggregate(data = Exp4, Response ~ Subject + VerbClass + Homogeneity + PrimeType, FUN = 'mean')
...
library(Rmisc)
Subj_noVerbClass <- aggregate(data = Exp4, Response ~ Subject+ Homogeneity + PrimeType, FUN = 'mean')
...
full<- glmer(Response ~ PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev
...
with(full@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)

##### Create model without PrimeType #####
NoPrimeType<- glmer(Response ~ Homogeneity.lev + PrimeType.lev:Homogeneity.lev
                         + (1 + PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev | Subject) + (1 + PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev | Item)
                         , data = Exp4, family = binomial, verbose=T, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

with(NoPrimeType@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)

##### Create model without Homogeneity #####
NoHomogeneity<- glmer(Response ~ PrimeType.lev + PrimeType.lev:Homogeneity.lev
                           + (1 + PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev | Subject) + (1 + PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev | Item)
                           , data = Exp4, family = binomial, verbose=T, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

with(NoHomogeneity@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)

##### Create model without PrimeType x Homogeneity interaction #####

NoInteraction<- glmer(Response ~ PrimeType.lev + Homogeneity.lev 
                           + (1 + PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev | Subject) + (1 + PrimeType.lev + Homogeneity.lev + PrimeType.lev:Homogeneity.lev | Item)
                           , data = Exp4, family = binomial, verbose=T, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))
...
MLR_PrimeType <- anova(full, NoPrimeType); capture.output(MLR_PrimeType, file = paste0(wd,exp, 'MLR_PrimeType.txt'))
MLR_Homogeneity <- anova(full, NoHomogeneity); capture.output(MLR_Homogeneity, file = paste0(wd,exp, 'MLR_Homogeneity.txt'))
MLR_interaction <- anova(full, NoInteraction); capture.output(MLR_interaction, file = paste0(wd,exp, 'MLR_interaction.txt'))
